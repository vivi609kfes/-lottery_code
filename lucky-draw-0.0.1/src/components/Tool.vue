    ...
    >
      <el-form ref="form" :model="form" label-width="80px" size="mini">
        <el-form-item label="抽取獎項">
          <el-select v-model="form.category" placeholder="請選擇本次抽取獎項">
            <el-option
              :label="item.label"
              :value="item.value"
        ...
        <el-form-item label="抽取方式">
          <el-select v-model="form.mode" placeholder="請選取本次抽獎方式">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (this.form.mode === 99) {
        if (this.form.qty <= 0) {
          return this.$message.error('必須輸入本次抽獎人數');
        }
        if (this.form.qty > this.remain) {
          return this.$message.error('本次抽獎人數已經超過本獎項的剩餘人數');
        }
      }
